...
{\tt edepmode} $=0$ mode is a simplified energy deposition mode available in
the Serpent Monte Carlo code.
In this mode, it is assumed that all energy deposited is due to fission,
multiplied
by an empirical factor taking into account other reactions. The formula is
$$
H_g=202.27 \ {\rm MeV}\times {Q_{\tt fiss}^{\tt er}\over
Q_{\tt fiss}^{\tt er,U235}} \, \sigma_{{\tt fiss},g}
$$
\noindent where $Q_{\tt fiss}^{\tt er,U235}$ is the pseudo-$Q$ value of
$^{235}$U. This option is activited by setting {\tt imode} $=1$ or $2$,
depending if the fission pseudo-$Q$ values are recovered from MF1/MT458 or
MF3/MT18, respectively.
